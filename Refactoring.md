...
- 특이 케이스 추가하기
  - 특이 케이스 객체 사용
  - 특이 케이스인지 검사하는 코드를 함수로 추출
- 제어 플래그를 탈출문으로 바꾸기
  - 제어 플래그: 코드의 동작을 변경하는 데 사용되는 변수, 보통 반복문에서 있는 경우가 많음
  - 모든 함수의 return문은 하나이기 보다는 할 일을 마칠 때 return으로 명확히 알리는 편이 낫다
